...
const UsersEdit = () => {
    ...
    const save = async (event) => {
        ...
        var {userName,nickname,password,level} = document.forms[0];
        var errors = "";
        errors += userName.value.length === 0 ? "Debe ingresarse un nombre.\n": "";
        errors += nickname.value.length === 0 ? "Debe ingresarse un nickname.\n": "";
        errors += password.value.length === 0 ? "Debe ingresarse una contraseña.\n": "";
        errors += level.value.length === 0 ? "Debe ingresarse un nivel.\n": "";
        if(errors.length > 0){
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({ "operatorId": config.operatorId, "name": userName.value, "nickname": nickname.value, 
